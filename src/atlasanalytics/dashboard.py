...
.sub { color: #5b6576; max-width: 760px; line-height: 1.6; }
.note { font-size: .82rem; color: #687386; }
.cards {
  display: grid;
...
.badge-investigate { background: #fef3c7; }
...
def build_dashboard_html(connection: duckdb.DuckDBPyConnection) -> str:
    ...
    issuer_rows = connection.execute(
        ...
        select
            ...
            sum(approved_attempts)::double
                / nullif(sum(authorization_attempts), 0) as approval_rate,
    ...
    issuer_html = "".join(
        ...
        f"<td>{int(row_attempts):,}</td>"
        f"<td>{_percent(float(rate or 0))}</td>"
        f"<td>{int(timeouts):,}</td>"
        f"<td>{float(latency or 0):.0f} ms</td>"
        "</tr>"
    ...
    anomaly_html = "".join(
        "<tr>"
        f"<td>{escape(str(day))}</td>"
        f"<td>{escape(str(name))}</td>"
        f"<td>{escape(str(currency))}</td>"
        f'<td><span class="badge badge-{escape(str(state))}">'
        f"{escape(str(state))}</span></td>"
    ...
    if not anomaly_html:
        anomaly_html = (
            '<tr><td colspan="6">'
            "No investigate/critical issuer-day signals in this dataset."
            "</td></tr>"
        )
...
<header>
  ...
  <p class="sub">
    Authorization health, issuer behavior and decline diagnostics generated directly
    from AtlasAnalytics marts. Values come from the deterministic synthetic dataset.
  </p>
</header>
<section class="cards">
  <div class="card">
    <span>Authorization attempts</span><strong>{attempts:,}</strong>
  </div>
  ...
  <div class="card">
    <span>Overall p95 latency</span><strong>{float(p95_latency or 0):.0f} ms</strong>
  </div>
</section>
<section class="grid">
  <div class="panel">
    <h2>Issuer performance</h2>
    <table>
      <thead><tr><th>Issuer</th><th>Currency</th><th>Attempts</th><th>Approval</th>
      <th>Timeouts</th><th>Peak daily p95</th></tr></thead>
      <tbody>{issuer_html}</tbody>
    </table>
  </div>
  <div class="panel"><h2>Decline mix</h2>{decline_html}</div>
</section>
<section class="panel full">
  <h2>Issuer anomaly signals</h2>
  <table>
    <thead><tr><th>Date</th><th>Issuer</th><th>Currency</th><th>State</th>
    <th>Approval z</th><th>Timeout z</th></tr></thead>
    <tbody>{anomaly_html}</tbody>
  </table>
  <p class="note">
    Rolling baselines use prior issuer observations only. A signal is a diagnostic flag,
    not proof of an outage or model failure.
  </p>
</section>
</main>
</body>
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate the AtlasAnalytics HTML operations dashboard"
    )
